...
ldapdelete -x -w $LDAPPASS -D "cn=root,dc=$DOMAIN,dc=$ORG" "uid=$USERNAME,ou=$UNIT,dc=$DOMAIN,dc=$ORG"

ldapdelete -x -w $LDAPPASS -D "cn=root,dc=$DOMAIN,dc=$ORG" "cn=$USERNAME,ou=$GROUP,dc=$DOMAIN,dc=$ORG"
