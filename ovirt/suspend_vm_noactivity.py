...
while True:
    try:
        ...
        vms = api.vms.list()
        print vms[0].name
        tagged_running_vms = [vm for vm in vms if vm.tags.list() != [] and vm.get_start_time() != None and vm.tags.list()[0].name == "1130teach"]
        print tagged_running_vms
        session_list = [ vm.sessions.list()[0].get_ip() != None for vm in tagged_running_vms ]
        time.sleep(600)
        session_list_later = [ vm.sessions.list()[0].get_ip() != None for vm in tagged_running_vms ]
